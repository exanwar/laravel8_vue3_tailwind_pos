...
export default {
    ...
    setup(props) {
        ...
        function getBrand(){
            axios.get('/api/user/product_brands/' + props.brand)
            .then(({data}) => {
                name.value = data.data[0].name
                image.value = data.data[0].brand_logo
            })
        }
    ...
    methods: {
        update(){
            let data = {
                name: this.name,
                brand_logo: this.image ? this.image : 'https://pixinvent.com/demo/vuexy-vuejs-admin-dashboard-template/demo-1/img/13-small.d796bffd.png'
            }
            axios.put('/api/user/product_brands/' + this.brand, data)
                .then( response => {
                    this.handleReload()
                    this.handleClose()
                    this.successToast('Brand has been updated!')
                })
        },
        handleClose() {
            this.$emit('close')
