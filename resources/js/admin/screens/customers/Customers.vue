        ...
        <div class="mt-0">
            <div
                class="
                    ...
                    font-sans
                "
                :class="$can('create_customers') ? 'flex-col lg:flex-row lg:space-x-5 justify-between overflow-hidden' : ''"
            >
                <div class="w-full" :class="$can('create_customers') ? ' lg:w-8/12 overflow-y-auto' : ''">
                    <div
                        class="
                                    mt-10
                                    flex
                    ...
                    <div class="bg-white shadow-md rounded mt-10">
                        <table class="w-full flex flex-row flex-no-wrap xl:bg-white rounded-lg overflow-hidden xl:shadow-xl">
                            ...
                            <tr class="bg-dashboard-blue flex flex-col flex-no-wrap xl:table-row rounded-l-lg xl:rounded-none mb-2 xl:mb-0" v-for="(item, index) in customers.data" :key="index">
                                ...
                                <th class="px-3 py-10 xl:py-3 text-left xl:text-center">Customer</th>
                            ...
                            <tr class="flex flex-col flex-no wrap xl:table-row mb-2 xl:mb-0" v-for="(item, index) in customers.data" :key="index">
                                ...
                                <td class="border-grey-light border hover:bg-gray-100 p-3 truncate">
                                    <div class="flex items-center">
                                        <div class="h-16 w-16 hidden lg:block">
                                        ...
                                        <div class="flex flex-col justify-start lg:ml-4">
                                            <h2 class="font-bold text-lg">{{ item.name }}</h2>
                                            <div>
                                                <p class="mb-0 font-semibold text-sm">{{ item.email_address }}</p>
                                                <p class="mb-0 italic font-medium text-xs">{{ item.phone }}</p>
                                                <p class="mb-0 italic font-medium text-xs">{{ item.address }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                ...
                <div class="w-full h-full overflow-hidden bg-white shadow-lg rounded-md" v-show="$can('create_customers')" :class="$can('create_customers') ? 'lg:w-4/12' : ''">
                    <div class="w-full
                                bg-dashboard-blue
                                bg-opacity-80
                                px-5
                                ...
                                justify-between
                                items-center">
                        <h1 class="text-gray-50 uppercase font-nunito tracking-wide font-bold" v-if="!edit">Add New Customer</h1>
                        <h1 class="text-gray-50 uppercase font-nunito tracking-wide font-bold" v-else>Edit Customer</h1>
                    </div>
                        ...
                        <div class="flex flex-col justify-center items-center">
                            <div class="relative mt-3">
                                ...
                                <div class="absolute left-0 top-0">
                                    <input type="file" accept=".jpeg, .png, .jpg" class="absolute h-40 w-40" style="opacity: 0;" @change="uploadCustomerPhoto">
                                </div>
...
export default {
    ...
    setup() {
        ...
        function getCustomers(page = 1){
            axios.get('/api/user/customers?page=' + page)
                .then(({data}) => {
        ...
        function deleteCustomer(id){
            InjectDirectly.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/user/customers/' + id)
                        .then( response => {
                            getCustomers()
        ...
        function searchIt(){
            if(query.value !== '') {
                axios.get('/api/user/find-customer?page=1&q=' + query.value )
                    .then( ({data}) => {
                        customers.value = data
                        last_page.value = data.meta.last_page
                        from.value = data.meta.from
        ...
        function openEdit(id){
            edit.value = true
            editId.value = id
            axios.get('/api/user/customers/' + id)
            .then(({data}) => {
        ...
        function update(){
            axios.put('/api/user/customers/' + editId.value, {
                name : name.value,
                image: image.value,
                email_address: email.value,
                phone: phone.value,
        ...
        async function customerPurchase(id){
            let purchased = null;
            await axios.get('/api/user/find-sell-by-customer/' + id)
            .then(({data}) => {
                console.log(data)
