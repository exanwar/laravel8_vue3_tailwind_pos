                    ...
                    <div class="bg-white rounded-lg overflow-hidden">
                        ...
                        >
                            <h1
                                ...
                                "
                            >
                                Edit Purchase Lot #{{ purchase_invoice_number }}
                            </h1>
                        </div>
...
export default {
    ...
    setup(props) {
        ...
        function getLot(){
            axios.get('/api/user/product_lots/' + props.lot)
            .then(({data}) => {
                let lotData = data.data[0]
                selectedVendor.value = lotData.vendor
                selectedProduct.value = lotData.product

                purchase_bill_number.value = lotData.bill_id
                purchase_invoice_number.value = lotData.invoice_id
                purchase_price.value = lotData.purchase_price_total
                purchase_quantity.value = lotData.product_quantity
                profit_in_percent.value = lotData.profit_in_percent
                overhead.value = lotData.overhead_price
                transportation_cost.value = lotData.transport_cost
                labour_cost.value = lotData.labour_cost
                other_costs.value = lotData.other_costs

                category_id.value = lotData.category.id
                unit_id.value = lotData.unit.id
                brand_id.value = lotData.brand.id
            })
        }
    ...
    methods: {
        submit(){
            let data = {
                ...
                category_id: this.selectedProduct ? this.selectedProduct.category.id : this.category_id,
                brand_id: this.selectedProduct ? this.selectedProduct.brand.id : this.brand_id,
                unit_id: this.selectedProduct ? this.selectedProduct.unit.id : this.unit_id,
                purchase: parseFloat(this.purchase_price).toFixed(2),
                qtn: this.purchase_quantity,
                buy_on: parseFloat(this.buy_each).toFixed(2),
                overhead: parseFloat(this.overhead).toFixed(2),
            ...
            axios
                .put("/api/user/product_lots/" + this.lot, data)
                .then(response => {
                    this.handleClose();
                    this.handleReload();
                    this.successToast('Purchase has been updated!')
                })
                .catch(error => {
                    // Swal.fire({
                    //     icon: "error",
